...
class KeyPair {
            ...
            case "secp256k1":
                ...
                if (key) {
                    if (key.indexOf("PRIVATE-") !== -1 || key.indexOf("PUBLIC-") !== -1) {
                        ...
                        if (key.indexOf("PRIVATE") == -1) {
                            this.key = this.curve.keyFromPublic(asn_1.AsnParser.decodeECPublicKey(key), "hex");
                        }
                        else {
                            this.key = this.curve.keyFromPrivate(asn_1.AsnParser.decodeECPrivateKey(key), "hex");
                        }
                    }
                }
                break;
    ...
    generate(bits = 2048) {
        ...
        switch (this.type) {
            case "rsa":
                ...
                handler.pub.hash = hash_1.Hash.getHash(handler.pub.pkcs8pem);
                handler.prv.hash = hash_1.Hash.getHash(handler.prv.pkcs8pem);
                return handler;
            case "bitcoin":
            case "ethereum":
            case "secp256k1":
                ...
                handler = {
                    pub: {
                        pkcs8pem: asn_1.AsnParser.encodeECPublicKey(this.key.getPublic("hex")),
                        hash: ""
                    },
                    prv: {
                        pkcs8pem: asn_1.AsnParser.encodeECPrivateKey(this.key.getPrivate("hex")),
                        hash: ""
                    }
                };
                handler.pub.hash = hash_1.Hash.getHash(handler.pub.pkcs8pem);
                ...
                return handler;
            default:
                throw activelogger_1.ActiveLogger.fatal(`Cannot generate ${this.type} key pair type`);
        }
    }
    encrypt(data, encoding = "base64") {
        ...
        throw activelogger_1.ActiveLogger.fatal(data, `Cannot encrypt with ${this.type}`);
    }
    decrypt(data, encoding = "base64") {
        if (this.type == "rsa" && this.rsa) {
            return this.rsa.decrypt(data, encoding).toString();
        }
        throw activelogger_1.ActiveLogger.fatal(data, `Cannot decrypt with ${this.type}`);
    }
    sign(data, encoding = "base64") {
        switch (this.type) {
            case "rsa":
                ...
                throw activelogger_1.ActiveLogger.fatal(data, `Failed to sign`);
            case "bitcoin":
            case "ethereum":
            case "secp256k1":
                if (this.key.priv) {
                    ...
                    data = hash_1.Hash.getHash(data);
                    return new Buffer(this.key.sign(data).toDER()).toString("base64");
                }
                throw activelogger_1.ActiveLogger.fatal(data, `Failed to sign`);
            default:
                throw activelogger_1.ActiveLogger.fatal(data, `Cannot sign with ${this.type}`);
        }
    }
    verify(data, signature, encoding = "base64") {
        switch (this.type) {
            case "rsa":
                ...
                throw activelogger_1.ActiveLogger.fatal(data, `Failed to verify with RSA`);
            case "bitcoin":
            case "ethereum":
            case "secp256k1":
                if (typeof data !== "string")
            ...
            default:
                throw activelogger_1.ActiveLogger.fatal(data, `Cannot verify with ${this.type}`);
        }
    }
}
